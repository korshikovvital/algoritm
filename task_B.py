def partition(competitors, left, right) -> int:
...
def quick_sort(competitors, left, right) -> None:
    if ((right - left) > 1):
...
def transformation(competitors) -> list:
...
if __name__ == '__main__':
    ...
    left = 0
    right = len(competitors)
    quick_sort(competitors, left, right)
    nlist = list(zip(*competitors))
    print(*(nlist[2]), sep="\n")